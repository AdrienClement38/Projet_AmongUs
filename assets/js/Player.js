...
export default class Player {
    ...
    constructor() {
        ...
        // correspondra à l'objet GameImage de l'image en cours
        this.current = this.base;
        // correspond à la position de l'image (dans le tableau)
        this.currentPos = 0;
        // Pour avoir un déplacement appréciable nous ne devons pas changer d'image à chaque Frame
    ...
    playerMove(move) {
        ...
        if (move.indexOf('ArrowLeft') !== -1) {
            this.position.scale.x = -1;
            this.position.x -= 5;
        }
        if (move.indexOf('ArrowRight') !== -1) {
            this.position.scale.x = 1;
            this.position.x += 5;
        }
